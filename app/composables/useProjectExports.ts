...
export const useProjectExports = async (projectId: number) => {
  const projectExports = useState<FullProjectExport[]>(
    "projectExports",
    () => [],
  );
  ...
  const totalPages = useState<number>("totalProjectPages", () =>
    Math.ceil(totalExports.value / pageSize),
  ); // should change after first fetch
  const exportsGenerating = useState<FullProjectExport[]>(
    "totalProjectExportsGenerating",
    () => [],
  );
  ...
  if (typeof projectId !== "number") {
    throw new Error(
      `Project id ${projectId} is not a number (useProjectExports)!`,
    );
  }
        ...
        .replaceAll(/\s/g, ""),
    {
      ...
      onResponse: async (context) => {
        if (context.response.status === 200) {
          ...
          const exportsFinished = exportsGenerating.value.filter(
            (ope) =>
              !json.projectExports.generating
                .map((npe) => npe.id)
                .includes(ope.id),
          );
          ...
          if (exportsFinished.length > 0) {
            ...
            const updatedFinished = await fetchByIds(
              exportsFinished.map((e) => e.id),
            );
      ...
      onResponseError: async (context) => {
        if (context.response.status === 401) {
          ...
          captureException(new Error("Export failure"), {
  ...
  async function deleteExport(exportId: number): Promise<void> {
  ...
  async function notifyOnDone(
    projectExports: FullProjectExport[],
  ): Promise<void> {
  ...
  async function fetchById(
    ...
    return $fetch<FullProjectExport>(
  ...
  async function fetchByIds(ids: number[]): Promise<FullProjectExport[]> {
  ...
  async function generateExport(params: ExportProjectParams) {
