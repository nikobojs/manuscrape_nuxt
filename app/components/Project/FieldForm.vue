...
const props = defineProps({
  ...
  onFieldUpdate:
    requireFunctionProp<(f: NewProjectFieldDraft) => void | Promise<void>>(),
  onFieldAdd:
    requireFunctionProp<(f: NewProjectFieldDraft) => void | Promise<void>>(),
...
const forceFieldRequired = computed<boolean>(() => {
  if (props?.fieldType && ["BOOLEAN"].includes(props.fieldType)) {
    props.onFieldUpdate({
      required: true,
      type: props.fieldType as FieldType,
      label: props.label,
...
const fieldTypeOptions = Object.entries(ObservationFieldTypes).map(
  ([key, val]) => ({
    label: key,
    type: val,
  }),
