    ...
    >
      ...
      <template #actions-data="{ row }">
        <div class="flex items-center justify-end relative w-9">
          <UIcon
            name="i-mdi-close"
            class="absolute right-2 cursor-pointer text-lg"
      ...
      <template #label-data="{ row }">
        ...
        <div class="flex items-baseline w-[257px]">
          <UTooltip>
            ...
            <span
              class="block relative whitespace-nowrap overflow-hidden text-ellipsis max-w-[256px]"
            >
              {{ row.label }}
            </span>
            <span
...
const props = defineProps({
  ...
  onFieldsUpdate:
    requireFunctionProp<(fields: NewProjectField[]) => void | Promise<void>>(),
});
